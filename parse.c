...
/*   Updated: 2023/10/06 10:28:14 by akhaliss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expand_var(t_envir *_env, char *s, size_t *i)
{
	...
	if (s[*i] == '$')
	{
		...
		if (_env->var && _env->value)
			...
			return (free(expanded), ft_strdup(""));
...
void	_delim(t_token **line, char *str, size_t *i)
{
	...
	if (flg == 0)
		token_tolist(line, node_token(s, NEXP_HERE));
	else
		token_tolist(line, node_token(s, EXP_HERE));
	free(s);	
